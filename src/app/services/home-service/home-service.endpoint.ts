...
export class HomeServiceEndpoint {
  ...
  private get getPashminaUrl() {
    return this.auth.getBaseUrl + this.pashminaUrl
  }

  private get getPashminaByIdUrl() {
    return this.auth.getBaseUrl + this.pashminaByIdUrl
  }

  private get getPashminaByCategoryUrl() {
    return this.auth.getBaseUrl + this.pashminaByCategoryUrl
  }
  ...
  public getPashminaByCategory<T>(category: string, pageNumber: number, pageSize: number) {
    return this.http.get<T>(this.getPashminaByCategoryUrl + "/" + pageSize + "/" + pageNumber, {
      params: {
        category: category
      }
    })
